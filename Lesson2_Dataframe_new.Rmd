...
```{r}
names <- c("Riona", "fiona", "Raj Kumar", "Jack Dorcy")
```
...
```{r}
names <- c("Apple", "Banana", "Guava", "Rock Melon", "Pineapple")
```
...
```{r}
fruit_rank <- data.frame(names, number)
```
...
```{r}
mutate(fruit_rank, number_in_10 = number + 10)
```
